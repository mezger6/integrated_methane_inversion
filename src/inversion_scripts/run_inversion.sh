...
python -u ${InvDir}/jacobian.py ${InvDir} ${invPath}/${configFile} $StartDate $EndDate $LonMinInvDomain $LonMaxInvDomain $LatMinInvDomain $LatMaxInvDomain $nElements $Species $satelliteCache $SatelliteProduct $UseWaterObs $isPost $period_i $buildJacobian False $FilterInland; wait
if "$LognormalErrors"; then
    # for lognormal error visualization of the prior we sample the prior run
    # without constructing the jacobian matrix
    python ${InvDir}/jacobian.py ${InvDir} ${invPath}/${configFile} $StartDate $EndDate $LonMinInvDomain $LonMaxInvDomain $LatMinInvDomain $LatMaxInvDomain $nElements $Species $satelliteCache $SatelliteProduct $UseWaterObs $isPost $period_i False True $FilterInland; wait
fi
